...
// function to copy old centroids
__global__ void copy_old_centroids(double **old_centroids, double **centroids, 
    ...
    while (index < num_clusters) {
        memcpy(old_centroids[index], centroids[index], dims*sizeof(double));
        index += totalthreads;
    }
}
...
// kernel to find the closest centroid
__global__ void find_nearest_centroids(double **points, double **centroids, int num_inputs, 
                                       int num_clusters, int dims, int* labels) {
    int totalthreads = blockDim.x*gridDim.x;
    int index = blockIdx.x*blockDim.x + threadIdx.x;
    while (index < num_inputs) {
        ...
        for (int i=0; i<num_clusters; i++) {
            ...
            for (int j=0; j<dims; j++) {
                distance += (centroids[i][j]-points[index][point_coord_pos])
                    *(centroids[i][j]-points[index][point_coord_pos]);
...
__global__ void find_point_sum(double **points, double**centroids, int *labels, 
    ...
    while (index < num_inputs) {
        int point_coord_pos = 0;
        for (int i=0; i<dims; i++) {
            my_atomicAdd(&(centroids[labels[index]][i]), points[index][point_coord_pos]);
            point_coord_pos += num_inputs;
        }
        atomicAdd(&(label_count[labels[index]]), 1);
        index += totalthreads;
    }
}
...
//         memcpy(host_centroids[i], host_points[index], dims*sizeof(double));
    }
    
    // do cuda mallocs and cuda memcpy together. It is faster
...
//                    dims*sizeof(double), cudaMemcpyHostToDevice);
    ...
    // space for storing intermediate distance while calculating nearest centroid
//     double *centroid_distances;
//     cudaMalloc((void**)&centroid_distances, num_inputs*num_clusters*sizeof(double));
    ...
    while (iters < max_num_iters) {
        ...
        find_nearest_centroids<<<blocks, threads_per_block>>>(device_points,
                                                              device_centroids,
                                                              num_inputs, num_clusters, 
                                                              dims, device_labels);
        threads_per_block = MAX_THREADS;
        blocks = min(MAX_BLOCKS, (num_clusters+threads_per_block-1)/threads_per_block);
        set_centroids_zero<<<blocks, threads_per_block>>>(device_centroids, 
                                                          num_clusters, dims, label_count);
        threads_per_block = 16;
        blocks = min(256, (num_inputs+threads_per_block-1)/threads_per_block);
        find_point_sum<<<blocks, threads_per_block>>>(device_points, device_centroids, device_labels, 
                       num_inputs, num_clusters, dims, label_count);
        
        threads_per_block = MAX_THREADS;
        blocks = min(MAX_BLOCKS, (num_clusters+threads_per_block-1)/threads_per_block);
        find_centroids<<<blocks, threads_per_block>>>(device_centroids, num_clusters, dims, label_count);
        iters++;
        // calculate convergence and break here if conditions are met
    ...
    // free memory here
    // Well don't free memory to save time
