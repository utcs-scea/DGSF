...
__global__ void find_nearest_centroids_kernel(REAL* dataset,
    ...
    __shared__ REAL t_centroids[MAX_CLUSTER_SIZE*MAX_DIMS_SIZE];

    int i = threadIdx.x + blockIdx.x * blockDim.x;
    int ti = threadIdx.x;
    
    if(num_cluster*dims < MAX_CLUSTER_SIZE*MAX_DIMS_SIZE){
        if(ti < num_cluster){
            for (int d = 0; d < dims; d++) {
                t_centroids[ti*dims+d] = centroids[ti*dims+d];
            }
        }   
    }
    __syncthreads();
    ...
    for (int k = 0; k < num_cluster; k++) {
        ...
        for (int d = 0; d < dims; d++) {
            if(num_cluster*dims < MAX_CLUSTER_SIZE*MAX_DIMS_SIZE){
                ...
                accum_diff += (dataset[i*dims+d] - t_centroids[k*dims+d]) * (dataset[i*dims+d] - t_centroids[k*dims+d]);
            }
        }
...
__global__ void average_labeled_centroids_kernel(REAL* centroids,
    ...
    __shared__ int temp[MAX_CLUSTER_SIZE];

    int i = threadIdx.x + blockIdx.x * blockDim.x;
    int ti = threadIdx.x;

    if(ti < num_cluster){
        temp[ti] = hit_counts[ti];
    }
    __syncthreads();
    ...
    if(i < num_cluster){
        for (int d = 0; d < dims; d++) {
            ...
            centroids[i*dims+d] = sum_centroids[i*dims+d] / temp[ti];
        }
    }
