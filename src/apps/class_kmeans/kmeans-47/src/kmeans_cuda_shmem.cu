...
{
   extern __shared__ float shared_mem_centers[];
   
   if (threadIdx.x < num_centroids) {
      for (int d=0; d < num_dims; d++) {
         shared_mem_centers[threadIdx.x * num_dims + d] = centers[threadIdx.x * num_dims + d];
      }
   }
   __syncthreads();
   ...
   if (point_id < num_points) {
      ...
      for (int k = 0; k < num_centroids; k++) {
         float distance = euclidean_distance_cuda(num_dims, shared_mem_centers, datapoints, point_id, k);
...
{      
   ...
   const int shmem_size = centers_size;
   ...
   for (int i=0; i < max_iterations; i++) {
      ...
      assign_labels_cuda<<<blocks_per_grid, threads_per_block, shmem_size>>>(num_points, num_dims, num_centroids, centers_device, datapoints_device, labels_device, sums_of_subsets_device, num_points_in_subsets_device);
