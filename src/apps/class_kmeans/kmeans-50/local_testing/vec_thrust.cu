...
struct point_add : public thrust::binary_function<thrust::device_ptr<double>,thrust::device_ptr<double>,thrust::device_ptr<double>>
{

  using Base = thrust::binary_function<thrust::device_ptr<double>,thrust::device_ptr<double>,thrust::device_ptr<double>>;
  int dims;
  thrust::device_ptr<double> temp_for_double_add;
  
  point_add(int dim, thrust::device_ptr<double>& temp_double_add) :
  Base(),
  dims(dim),
  temp_for_double_add(temp_double_add) {}
  
  __host__ __device__
  thrust::device_ptr<double> operator()(const thrust::device_ptr<double>& x,
                                        const thrust::device_ptr<double>& y) {
    //thrust::device_ptr<double> c = x;
    //double *a = thrust::raw_pointer_cast(x);
    //double *b = thrust::raw_pointer_cast(y);
    for (int i = 0; i < dims; i++) {
    
        printf("%d %x %x %f %f  %x  %f %f\n", i, x, y, x[i], y[i],
        temp_for_double_add, temp_for_double_add[i],
        x[i] + y[i]);
    
        //printf("values: %d %f %f", i, a[i], b[i]);
        //thrust::raw_pointer_cast(temp_for_double_add)[i] = a[i] + b[i];
       //if (x[i] + y[i] > 10000000) {
    //      printf("awesome");
      // }
    }
 ...
 struct saxpy2 {
     thrust::device_ptr<double>* k_centroids;
     int num_cluster;
     int dim;
     saxpy2(thrust::device_ptr<double>* centroids,
           int n_cluster,
           int dim):
     ...
     __host__ __device__
     double getDistanceSq(const thrust::device_ptr<double>& point1, const thrust::device_ptr<double>& point2) const {
     ...
     __host__ __device__
     int operator()(const thrust::device_ptr<double> &point) const {
        int nearest_index = 0;
        double smallest_dist =  getDistanceSq(point, k_centroids[nearest_index]);
         //const thrust::device_ptr<double>& point2 =  k_centroids + nearest_index;
...
void read_file(struct options_t& options,
               int*              n_vals,
               thrust::device_vector<thrust::device_ptr<double>>& data_points,
               thrust::device_vector<thrust::device_ptr<double>>& k_centroids,
               thrust::device_vector<thrust::device_ptr<double>>& k_old_centroids,
    ...
    data_points = thrust::device_vector<thrust::device_ptr<double>>(*n_vals);
    ...
    thrust::device_ptr<double> all_centroid_points = thrust::device_malloc<double>(options.num_cluster * options.dims);
    k_centroids = thrust::device_vector<thrust::device_ptr<double>>(options.num_cluster);
    ...
    thrust::device_ptr<double> all_sum_centroid_points = thrust::device_malloc<double>(*n_vals * options.dims);
    k_old_centroids = thrust::device_vector<thrust::device_ptr<double>>(*n_vals);
...
int main(int argc, char** argv) {
    ...
    thrust::device_ptr<double> all_points = thrust::device_malloc<double>(num_vals * options.dims);
    data_points = thrust::device_vector<thrust::device_ptr<double>>(num_vals);
    data_points[0] = all_points;
...
/*
    ...
    for (int i = 0; i < num_vals; ++i) {
        for (int j=0; j < options.dims; j++) {
            thrust::device_ptr<double> temp = data_points[i];
			std::cout << a[j] << " ";
		}
        std::cout << std::endl;
    }
*/    
    ...
    for (int i = 0; i < options.num_cluster; i++) {
        for (int j=0; j < options.dims; j++) {
            thrust::device_ptr<double> temp = k_centroids[i];
			std::cout << temp[j] << " ";
		}
        std::cout << std::endl;
    }
    ...
    thrust::device_ptr<double> temp_for_double_add = thrust::device_malloc<double>(options.dims);
    ...
    for(int i=0; i < options.max_num_iter; i++) {
        ...
        std::cout << data_points.size() <<
            " clusterid_of_points:" << clusterid_of_points.size() <<
            " k_centroids:" << k_centroids.size() <<
            " k_old_centroids:" << k_old_centroids.size() <<
            " index_of_each_cluster:" << index_of_each_cluster.size();
