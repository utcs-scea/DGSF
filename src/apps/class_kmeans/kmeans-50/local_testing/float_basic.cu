...
__global__ void setCluster(double* cuda_input_vals,
    ...
    int centroid_index = getNearestCentroid(cuda_input_vals,
     ...
     for(int z=0; z < dims; z++) {
       atomicAdd(&cuda_k_centroids_sum[centroid_index*dims + z], cuda_input_vals[row_id + z]);
     }
...
/*
    ...
    */
    ...
    float* cuda_k_centroids_sum;
    cudaMalloc((void**)&cuda_k_centroids_sum, options.num_cluster * options.dims * sizeof(float));    
    ...
    int num_threads = (options.num_cluster > 64) ? options.num_cluster : 64;
    ...
    for(; iter < options.max_num_iter; iter++) {
        cudaMemset(cuda_k_centroids_sum, 0, options.num_cluster * options.dims * sizeof(float));
      ...
      /*  
        ...
        for(int j=0; j < num_vals; j++) {
            ...
            cuda_num_points_each_cluster[centroid_index]++;
        ...
        for(int j =0; j < options.num_cluster; j++) {
            if (cuda_num_points_each_cluster[j] != 0) {
                for(int z=0; z < options.dims; z++) {
                    k_centroids[j][z] = k_centroids[j][z]/cuda_num_points_each_cluster[j];
                }
            }
        }
