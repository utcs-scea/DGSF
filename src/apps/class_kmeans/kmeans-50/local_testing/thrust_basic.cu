...
__device__ int getNearestCentroid(double* cuda_input_vals,
    ...
    //double* point, double** k_centroids, int k, int dim) {
    int nearest_index = 0;
...
__global__ void computeCentroid(double* cuda_k_centroids,
    ...
    for(int i=0; i < dims; i++) {
        cuda_k_centroids[index*dims + i] = cuda_k_centroids_sum[index*dims + i]/total_points;
    }
...
int main(int argc, char** argv) {
	thrust::host_vector<thrust::host_vector<int>> H;
    ...
    int num_threads = options.num_cluster > 1024 ? options.num_cluster : 1024;
