...
int main(int argc, char **argv) 
{
   ...
   float cuda_time_per_iter_in_ms;
   float cuda_total_time_in_ms;
   std::tie(iter_to_converge, cuda_total_time_in_ms) = k_means_cuda(opts.n_cluster, opts.n_dims, opts.max_iter, opts.threshold, n_points, flattened_centers, flattened_datapoints, labels); 

   //End timer and print out elapsed
   auto end = std::chrono::high_resolution_clock::now();
   auto diff = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
//    std::cout << "total time in milliseconds: " << diff.count() << std::endl;
//    std::cout << "cuda total time in milliseconds: " << cuda_total_time_in_ms << std::endl;   
   
   cuda_time_per_iter_in_ms = cuda_total_time_in_ms / iter_to_converge;
   
   printf("%d,%lf\n", iter_to_converge, cuda_time_per_iter_in_ms);
