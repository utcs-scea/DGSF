...
mkdir -p ${model_dir}
# onnxruntime model
cp "${SCRIPT_DIR}/model/updated_arcfaceresnet100-8.onnx" \
    ${model_dir}

data_dir="${MNT_DIR}/data"
mkdir -p ${data_dir}
cp "${SCRIPT_DIR}/../faas_face_id_client/input_data/pairs.txt" ${data_dir}/
rsync -av "${SCRIPT_DIR}/../faas_face_id_client/input_data/lfw_faces/" ${data_dir}/