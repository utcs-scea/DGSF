...
class RetinaFace(object):
    ...
    def onnxrt_init(self, modelio, cpu_threads: int,
        ...
        if no_sess:
            ...
            self.sess = rt.InferenceSession(modelio.getbuffer() , providers=providers,
                                            sess_options=sess_options)
...
def main():
    ...
    parser.add_argument('--mpath', required=True,
    ...
    modeldl_start = timer()
    model = io.BytesIO()
    s3_client.download_fileobj("hf-dgsf", "face_det/updated_withpreprop_r50.onnx", model)
    model.seek(0)
    modeldl_end = timer()

    modelload_start = timer()
    model = RetinaFace(model, args.network, args.bsize,
    ...
    for i in range(0, length, bs):
        ...
        padded_img_list, rewrite_list, dl = get_image_batch(sub_list, args.dataset_path)

        
        loc_b, conf_b, landmarks_b, inf = \
