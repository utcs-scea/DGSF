...
def get_zoom_function(cubin_dir):
    kern_mod = cupy.RawModule(path=os.path.join(cubin_dir, 'zoom_kernel.cubin'))
    print(" >>> reading zoom kernel from ", os.path.join(cubin_dir, 'zoom_kernel.cubin'), file=sys.stderr, flush=True )
