...
input_fname = os.path.join("/tmp", input_fname)

SCRIPT_NAME = "kmeans"
EXTRA_ARGS = "--type raw --seed 123 -i " + input_fname
DUMPS_PREFIX = "cuda_dumps/kmeans"
DUMPS_DIR = "/tmp/kmeans/cuda_dumps"
HOST_AVA_LIBS = "/home/ubuntu/serverless-gpus/build/ava/release/onnx_opt/lib"
WORKER_DUMPS_DIR = "/home/ubuntu/serverless-gpus/src/apps/kmeans/cuda_dumps"
REQ_MEMORY = "350"
...
def handler(req, _):
    ...
    s3_client.download_file('hf-dgsf', 'kmeans/1m_16d_16c.txt', input_fname)
    end_download = timer()
    ...
    ret = {}
    ret["execution"] = 0
    ret["download_model"] = 0
    ret["load_model"] = 0
    ret["download_input"] = round((end_download-start_download)*1000, 2)
    qtime = None
    for line in stdout.splitlines(): 
        print("line: ", line)
        if line.startswith("cuda_init,"):
            ret["cuda_init"] = float(line.split(",")[1][:-3])
        elif line.startswith("read_input,"):
            ret["execution"] += float(line.split(",")[1][:-3])
        elif line.startswith("execution,"):
            ret["execution"] += float(line.split(",")[1][:-3])
        elif line.startswith("end-to-end,"):
            ret["end-to-end"] = float(line.split(",")[1][:-3])
